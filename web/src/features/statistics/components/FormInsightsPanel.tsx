...
function DonutChart({
  ...
  if (total === 0) {
    return <div className="flex items-center justify-center h-32 text-xs text-stitch-on-surface-variant">Sin datos</div>;
  }
    ...
    <div className="relative w-32 h-32 mx-auto">
      ...
      >
        <div className="absolute inset-3 bg-[#141720] rounded-full flex flex-col items-center justify-center">
          <span className="text-lg font-headline font-bold text-white">{centerValue}</span>
          <span className="text-[9px] text-stitch-on-surface-variant uppercase font-label">{centerLabel}</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-[#141720] border border-[#2D3A4A] rounded-[12px] p-5">
      <div className="flex items-center gap-2 mb-6">
        <span className="material-symbols-outlined text-[#F5A623] text-lg">insights</span>
            ...
            {data.weaponType.map((w) => (
              <div key={w.label} className="flex items-center gap-2 text-[11px]">
                <span
                  ...
                  style={{ backgroundColor: WEAPON_COLORS[w.label] ?? '#6B7A8D' }}
                />
                <span className="text-stitch-on-surface-variant">{w.label}</span>
                <span className="ml-auto text-white font-bold">{w.pct}%</span>
              </div>
            ))}
          </div>
          {data.weaponType.find((w) => w.label === 'Arma de fuego' && w.pct > 50) && (
            <div className="mt-2 bg-red-900/30 border border-red-500/30 rounded-lg px-3 py-2">
              <div className="flex items-start gap-2">
                ...
                <p className="text-[10px] text-stitch-error leading-relaxed">
                  Dato para MININTER: Lima supera el 50% de robos con arma de fuego
                </p>
              </div>
            </div>
          )}
        ...
        <div className="flex flex-col gap-3 relative">
          ...
          <div className="flex-1 flex flex-col items-center justify-center relative">
            ...
            <div className="flex items-center gap-2 mt-3 bg-blue-900/20 border border-blue-500/30 rounded-lg px-3 py-2 z-10">
              <span className="material-symbols-outlined text-[16px] text-blue-400">hourglass</span>
