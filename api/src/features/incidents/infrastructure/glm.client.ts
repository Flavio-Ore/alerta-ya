...
export async function analyzeHistoricalData(
  ...
  const top = context.districts
    .slice(0, 15)
    .map((d, i) => `${i + 1}. ${d.district}: riesgo ${d.risk}/100 (${d.count} denuncias)`)
    .join('\n');
  const types = context.types.map((t) => `- ${t.type}: ${t.count}`).join('\n');

  const system =
    'Sos un analista de seguridad ciudadana para autoridades de Lima, Perú. ' +
    'Tu BASE son los datos históricos provistos (denuncias DataCrim/INEI 2017–2020). ' +
    'Podés usar búsqueda web SOLO para contexto ACTUAL y factual relevante a seguridad ' +
    ...
    '(2) DISTINGUÍ SIEMPRE la fuente: marcá "[Histórico]" para conclusiones de los datos y ' +
    ...
    '(4) Si la web no aporta nada confiable, omití esa parte. ' +
    '(5) Los datos NO tienen hora del día: hablá de DÓNDE, nunca de a qué hora. ' +
    '(6) Sos asesor, la autoridad decide. Respondé breve, en español, máximo 200 palabras.';
